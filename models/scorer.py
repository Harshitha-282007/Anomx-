...
MODEL_PATH = str((PROJECT_ROOT / config["paths"]["trained_model_dir"]).resolve())
...
class ForexGuardScorer:
    ...
    def _load_lstm(self, path: str):
        ...
        self.lstm_model.load_state_dict(
            torch.load(model_dir / "lstm_autoencoder.pt", map_location="cpu")
        )
        self.lstm_model.eval()

        self.lstm_scaler    = joblib.load(model_dir / "lstm_scaler.pkl")
        self.lstm_threshold = joblib.load(model_dir / "lstm_threshold.pkl")
